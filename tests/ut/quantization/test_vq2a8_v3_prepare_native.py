...
""".replace("== 1e-12f", f"== {minimum_scale!r}f")
    ...
    def test_fp8_conversion_uses_existing_a5_recipe(self):
        ...
        # Official v0.23.0's grouped-quant recipe uses the same saturating
        # float-to-byte cast and packed stores; mla_prolog_v3 arrived later.
        precedent = (REPO / "csrc/moe/swiglu_group_quant/op_kernel/swiglu_group_quant_base.h").read_text(
            encoding="utf-8"
        )
        for expression in (
            "RegLayout::ZERO",
