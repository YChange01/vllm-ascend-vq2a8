...
def test_profiler_timeout_kills_only_owned_process_group(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(profile.os, "killpg", lambda pid, sig: killed.append(pid), raising=False)
    monkeypatch.setattr(profile.signal, "SIGKILL", 9, raising=False)
...
def test_progress_poll_does_not_reset_deadline_or_kill_running_child(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        profile.os, "killpg", lambda *a: pytest.fail("Progress poll is not the overall deadline"), raising=False
    )
