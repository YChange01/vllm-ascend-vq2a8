...
def test_native_model_options_are_explicit_and_default_unchanged(bad, tmp_path):
    assert (
    ...
    plan = offline_engine_options(
        ...
        ascendc_library=str(tmp_path / "libvq2a8_ascendc.so"),
        ascendc_sha256="a" * 64,
    )
    cfg = config()
...
def construct_meta_root(hf_config, root_linear_mode="bf16"):
    ...
    for name in ("SupportsPP", "DeepseekV2MixtureOfExperts", "SupportsLoRA", "SupportsEagle3", "EagleModelMixin"):
        scope[name] = type(name, (), {})
    scope["EagleModelMixin"].aux_hidden_state_layers = ()
