...
@pytest.mark.parametrize("rank", [0, 1])
@pytest.mark.parametrize("physical_devices", [(0, 1), (2, 3)])
def test_tp2_owner_selects_rank_artifact_and_runtime_without_tp1_reader(
    tmp_path, monkeypatch, capsys, rank, physical_devices
):
    ...
    def device_id_to_physical_device_id(index):
        mapped_logical_devices.append(index)
        return physical_devices[index]

    # Expose only the official vLLM 0.23 API. The 0.26-only visible_device_*
    # spelling must fail here rather than being hidden by a permissive stub.
    monkeypatch.setitem(
        sys.modules,
        "vllm.platforms",
        NS(current_platform=NS(device_id_to_physical_device_id=device_id_to_physical_device_id)),
    )
    ...
    assert mapped_logical_devices == [rank]
    assert (
        f"tp_rank={rank} tp_size=2 logical_device={rank} physical_device={physical_devices[rank]}"
