...
"""Candidate F exact M=1 FP8-byte reorder gate, not performance acceptance.

Reuses the tail-reorder probe's fixture, byte/descriptor checks and bounded
...
def parse_args(argv=None):
    ...
    parser.add_argument("--library", type=Path, default=REPO / "build/vq2a8-ascendc-v4-v2-row-reuse" / LIBRARY_NAME)
...
def require_abi(native):
    try:
        version = native.activation_reorder_row_reuse_version()
    except (AttributeError, RuntimeError) as error:
        raise RuntimeError("Row-reuse reorder ABI missing; no fallback") from error
    if type(version) is not int or version != 1:
        raise RuntimeError(f"Row-reuse reorder requires independent ABI 1, got {version!r}")
...
def check_prepared(fixture, prepared, oracle, descriptors, name, *, rank3=False):
    if not isinstance(prepared, (tuple, list)) or len(prepared) != 4:
        raise AssertionError(f"{name}: prepare_row_reuse must expose four outputs")
    if rank3:
        groups = len(oracle[0])
...
def checked_case(fixture, inputs, literal_ids, owner, name, *, projection=True):
    ...
    prepared = fixture.bank.prepare_row_reuse(*inputs)
    descriptors = common.descriptor_reference(fixture, inputs, literal_ids, prepared)
    projected = fixture.bank.project_row_reuse(*inputs) if projection else None
                ...
                for routes in ("sequential", "duplicate"):
                    for rank in (2, 3):
                        ...
                        with stage(name):
                            ...
                            checked_case(fixture, inputs, ids, owner, name, projection=pattern == "finite")
                        names.append(name)
    return names
...
def run_contract_checks(fixture, stage):
    ...
    with stage("native_metadata_rejections"):
        for method in (fixture.bank.prepare_row_reuse, fixture.bank.project_row_reuse):
            for values in bad_inputs:
                try:
                    ...
                    raise AssertionError("Row-reuse native binding accepted invalid metadata")
            with torch.npu.stream(torch.npu.Stream()):
                try:
                    method(x, scale, bias, ids)
                except RuntimeError:
                    count += 1
                else:
                    raise AssertionError("Row-reuse native binding accepted a different bank stream")
    if count != CONTRACT_CASES:
        raise AssertionError("Row-reuse native contract coverage incomplete")
    return count
        ...
        for groups in (1, 6):
            ...
            with torch.npu.stream(stream):
                with stage(f"graph_k{k}_g{groups}_prepare"):
                    ...
                    for _ in range(2):
                        fixture.bank.prepare_row_reuse(*static)
                        fixture.bank.project_row_reuse(*static)
                    torch.npu.synchronize()
                    graph = torch.npu.NPUGraph()
                    with torch.npu.graph(graph, stream=stream):
                        prepared = fixture.bank.prepare_row_reuse(*static)
                        projected = fixture.bank.project_row_reuse(*static)
...
def run_queue_checks(device, factory, stage):
    ...
    with stage("queue_owner_release_and_allocation_pressure"):
        for iteration in range(QUEUE_ITERATIONS):
            ...
            projected = fixture.bank.project_row_reuse(*inputs) if oracle[2] is not None else None
...
def validate_child_evidence(result, *, queue_lifetime):
    ...
    if (
        ...
        raise ValueError("Incomplete row-reuse child evidence")
...
def run_case_child(args):
    ...
    try:
        ...
        with stage("library"):
            ...
            emit(CASE, "INFO", library=identity, device=info, native_abi=1, dispatch_scope=DISPATCH_SCOPE)
...
def main(argv=None):
    ...
    if args.plan_only:
        ...
        return 0
    if os.name != "posix":
        raise RuntimeError("Row-reuse validation requires Linux + NPU; use --plan-only elsewhere")
    directory = args.report_dir or Path(tempfile.mkdtemp(prefix="vq2-row-reuse-"))
    ...
    try:
        ...
        print(f"V4_REORDER_ROW_REUSE={report['status']} SUMMARY={summary}", flush=True)
