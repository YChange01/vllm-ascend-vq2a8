...
class custom_build_info(build_py):
    def run(self):
        gen_build_info()
        super().run()
        copy_vq2a8_notice(os.path.join(self.build_lib, "vllm_ascend"))


def copy_vq2a8_notice(package_dir):
    """Keep the provenance warning with every packaged VQ2A8 native library."""
    library = os.path.join(package_dir, "libvq2a8_ascendc_v4_v2.so")
    if not os.path.isfile(library):
        return
    # build_py normally precedes build_ext, so package_data alone cannot pick
    # up the notice generated by a first native install. Copy from the source
    # notice after either step, and fail the build if that notice is absent.
    # This warning is not permission to redistribute the derived kernel.
    source = os.path.join(ROOT_DIR, "csrc", "vq2a8_ascendc_v4_v2", "NOTICE")
    shutil.copyfile(source, os.path.join(package_dir, "VQ2A8_NOTICE"))
...
class cmake_build_ext(build_ext):
    ...
    def configure(self, ext: CMakeExtension) -> None:
        ...
        cmake_args += [f"-DVLLM_ASCEND_BUILD_VQ2A8={'ON' if envs.VLLM_ASCEND_BUILD_VQ2A8 else 'OFF'}"]
...
setup(
    ...
    package_data={"vllm_ascend": ["libvq2a8_ascendc_v4_v2.so", "VQ2A8_NOTICE"]},
