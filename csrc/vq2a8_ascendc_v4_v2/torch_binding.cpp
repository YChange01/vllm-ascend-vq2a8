...
class ResidentBank : public torch::CustomClassHolder {
 public:
  ...
  Tensors ProjectVectorized(const at::Tensor& x, const at::Tensor& scale, const at::Tensor& bias, const at::Tensor& ids) {
    const auto state = state_;
    const c10_npu::OptionalNPUGuard guard(state->table.device());
    CheckIds(ids);
    ...
    const uint32_t prepareWork = static_cast<uint32_t>(routes * m) * std::max(state->n, state->k) / kSelectColumns;
    ...
    at_npu::native::OpCommand::RunOpApi(
        "Vq2a8AscendCV4V2ProjectionVectorized",
