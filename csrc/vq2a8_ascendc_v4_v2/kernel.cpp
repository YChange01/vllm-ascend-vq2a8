...
// VQ2A8 kernel v2. Source rights remain unchanged; see NOTICE.
 ...
 public:
  ...
  __aicore__ inline void Process(GM_ADDR descriptors, uint32_t jobs, uint32_t nTiles, uint32_t cores) {
    ...
    for (uint32_t work = core; work < jobs * nTiles; work += cores) {
      // All three peers use the same job-major order, including skipped routes.
      const uint32_t descriptorWork = work;
