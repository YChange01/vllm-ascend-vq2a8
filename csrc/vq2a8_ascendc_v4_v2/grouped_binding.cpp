...
void CheckProjection(const at::Tensor& x, const at::Tensor& scale, const at::Tensor& bias, const at::Tensor& packed,
  ...
  TORCH_CHECK(x.defined(), "activation: undefined tensor");
...
std::vector<at::Tensor> GroupedProjection(const std::vector<at::Tensor>& x, const std::vector<at::Tensor>& scale,
  ...
  // Standalone KERNEL PROBE ONLY: at most 432 metadata bytes H2D. Each expert keeps its independent storage;
...
TORCH_LIBRARY(vq2a8_ascendc_v4_v2, m) {
  m.def(
      "grouped_projection(Tensor[] x, Tensor[] scale, Tensor[] bias, Tensor[] packed_zn, Tensor[] pair_lut) -> "
      "Tensor[]");
  m.def("abi_version() -> int", []() -> int64_t { return vq2a8_ascendc_v4_v2::kAbiVersion; });
}
TORCH_LIBRARY_IMPL(vq2a8_ascendc_v4_v2, PrivateUse1, m) {
  m.impl("grouped_projection", &vq2a8_ascendc_v4_v2::GroupedProjection);
}
