...
void CheckInput(const at::Tensor& hidden, const at::Tensor& ids, const at::Tensor& table, uint32_t width) {
  TORCH_CHECK(hidden.defined() && table.defined() &&
              ...
              "Resident SwiGLU/select/sign requires the bank NPU");
  TORCH_CHECK(hidden.dim() == 2 && hidden.scalar_type() == at::kBFloat16 &&
                  hidden.size(0) >= 1 && hidden.size(0) <= 6 && hidden.size(1) == 2 * width &&
                  (width == 2048 || width == 4096), "Resident SwiGLU/select/sign requires BF16[G,2K], G1..6, K2048/4096");
  TORCH_CHECK(hidden.stride(1) == 1 && (hidden.stride(0) == 0 || hidden.stride(0) >= 2 * width),
              "Resident SwiGLU/select/sign requires unit columns and expanded or nonoverlapping rows");
  ...
  TORCH_CHECK(reinterpret_cast<uintptr_t>(hidden.data_ptr()) % 32 == 0 &&
              ...
              "Resident SwiGLU/select/sign input rows require 32-byte alignment");
  ...
  TORCH_CHECK(hidden.storage_offset() >= 0 && static_cast<uint64_t>(hidden.storage_offset()) <= storageElements,
              "Resident SwiGLU/select/sign input storage offset is invalid");
  const uint64_t available = storageElements - hidden.storage_offset();
  TORCH_CHECK(2 * width <= available && (hidden.size(0) == 1 ||
                  rowStride <= (available - 2 * width) / static_cast<uint64_t>(hidden.size(0) - 1)),
              "Resident SwiGLU/select/sign input row span exceeds storage");
  TORCH_CHECK(ids.defined() && ids.device() == table.device() && ids.scalar_type() == at::kLong &&
              ...
              "Resident SwiGLU/select/sign requires contiguous INT64 slots[G] on the bank device");
  ...
  TORCH_CHECK(ids.storage_offset() >= 0 && static_cast<uint64_t>(ids.storage_offset()) <= idElements &&
              ...
              "Resident SwiGLU/select/sign slot storage is invalid");
}
...
std::vector<at::Tensor> ResidentSwigluSelectSign(
  ...
  TORCH_CHECK(std::isfinite(limit) && limit >= 0 && limit <= std::numeric_limits<float>::max(),
              "Resident SwiGLU/select/sign limit must be finite nonnegative FP32; zero disables clamp");
  TORCH_CHECK(experts >= 1 && experts <= 256 && scaleOwners.size() == experts &&
                  biasOwners.size() == experts && signOwners.size() == experts,
              "Resident SwiGLU/select/sign requires complete immutable bank owners");
  const c10_npu::OptionalNPUGuard guard(table.device());
  const char* soc = aclrtGetSocName();
  TORCH_CHECK(soc && std::strncmp(soc, "Ascend950", 9) == 0, "Resident SwiGLU/select/sign requires Ascend950");
  auto* platform = platform_ascendc::PlatformAscendCManager::GetInstance();
  TORCH_CHECK(platform && platform->GetCoreNumAiv() > 0, "Cannot query Ascend vector cores");
  auto signedOutput = at::empty({hidden.size(0), int64_t(width)}, hidden.options().dtype(at::kFloat));
  ...
  TORCH_CHECK(launchStream == bankStream, "Resident SwiGLU/select/sign must use the bank creation stream");
  ...
  const float clampLimit = static_cast<float>(limit);
  at_npu::native::OpCommand::RunOpApi("Vq2a8V4V2ResidentSwigluSelectSign",
      [launchStream, blocks, table, ids, hidden, signedOutput, selectedScale, selectedBias, selectStatus,
       inputStatus, scaleOwners, biasOwners, signOwners, experts, groups, width, rowStride, clampLimit]() -> int {
    ...
    LaunchResidentSwigluSelectSign(launchStream, blocks, table.data_ptr(), ids.data_ptr(), hidden.data_ptr(),
        signedOutput.data_ptr(), selectedScale.data_ptr(), selectedBias.data_ptr(), selectStatus.data_ptr(),
        inputStatus.data_ptr(), experts, groups, width, rowStride, clampLimit);
    return 0;
  }, false);
...
TORCH_LIBRARY_FRAGMENT(vq2a8_ascendc_v4_v2, m) {
  m.def("swiglu_select_sign_version() -> int", []() -> int64_t { return 1; });
}
