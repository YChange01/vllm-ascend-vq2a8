...
class LayerValidityKernel {
 public:
  __aicore__ inline void Init(GM_ADDR result) {
    ...
    pipe_.InitBuffer(reducedUb_, kValidityScalarBytes);
    valid_ = true;
  }
  ...
  __aicore__ inline void CheckOutput(GM_ADDR pointer, uint32_t rows, uint32_t width) {
    ...
    for (uint32_t row = 0; row < rows; ++row) {
      ...
      PipeBarrier<PIPE_V>();
      // Unsigned min equals UINT32_MAX iff every mask word equals UINT32_MAX.
      // Keep the original vectorized validity predicate and reduction order.
      ...
      const uint32_t allBits = reducedUb_.Get<uint32_t>().GetValue(0);
...
extern "C" __global__ __aicore__ void vq2a8_v4_v2_layer_validity_vectorized(
  ...
  vq2a8_ascendc_v4_v2::RunLayerValidity(s0, s1, s2, s3, s4, s5, gate, down, combined,
