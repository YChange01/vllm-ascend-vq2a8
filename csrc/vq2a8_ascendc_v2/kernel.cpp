...
// VQ2A8 kernel v2. Source rights remain unchanged; see ../vq2a8_expert_reference/README.md.
...
extern "C" __global__ __aicore__ void vq2a8_ascendc_v2_grouped(GM_ADDR descriptors, uint32_t jobs, uint32_t nTiles,
                                                               uint32_t cores) {
  KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
  vq2a8_ascendc_v2::AscendCV2Projection kernel;
  kernel.Init();
...
void LaunchGrouped(void* stream, uint32_t blocks, void* descriptors, uint32_t jobs, uint32_t nTiles) {
  vq2a8_ascendc_v2_grouped<<<blocks, nullptr, stream>>>(static_cast<GM_ADDR>(descriptors), jobs, nTiles, blocks);
}
}  // namespace vq2a8_ascendc_v2
