...
extern "C" __global__ __aicore__ void vq2a8_ascendc_resident_select(GM_ADDR bank, GM_ADDR routeIds, GM_ADDR scale,
                                                                    GM_ADDR bias, GM_ADDR sign, GM_ADDR valid,
                                                                    uint32_t experts, uint32_t routes, uint32_t k) {
  KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_AIV_ONLY);
  vq2a8_ascendc::ResidentSelectKernel op;
...
void LaunchResidentSelect(void* stream, uint32_t blocks, void* bank, void* routeIds, void* scale, void* bias,
  ...
  vq2a8_ascendc_resident_select<<<blocks, nullptr, stream>>>(
