...
#include "launch.h"
#define VQ2A8_V3_LAYOUT_FN __aicore__ inline
#include "layout.h"
...
// Independent v3 branch of v1. Original K128 Cube accumulation and FP32
// scale -> bias -> BF16 order are retained. Prepared jobs DMA-load constant
// tables; M1 bypasses repeated padded activation zero-fill and Gather.
// Cube remains M32: this is NOT a claimed native M1/GEMV implementation.
// A5-only: vector UB lookup and optional L1 ping-pong.
...
class ProjectionKernel {
 ...
 public:
  ...
  __aicore__ inline void InitBuffers(GM_ADDR constants = nullptr) {
    ...
    if ASCEND_IS_AIV {
      if (constants != nullptr) {
        // Six aligned contiguous DMA copies replace 6656 scalar stores.
        // Signed shift tensors receive the same raw nonnegative int32 bits.
        GlobalTensor<uint32_t> words;
        words.SetGlobalBuffer(reinterpret_cast<__gm__ uint32_t*>(constants));
        DataCopy(ndOffsets_.Get<uint32_t>(), words[kNdConstants], kHalfTileBytes / 4);
        DataCopy(packedOffsets_.Get<uint32_t>(), words[kPackedConstants], kPairs);
        DataCopy(idOffsets_.Get<uint32_t>(), words[kIdConstants], kPairs);
        DataCopy(codeShifts_.Get<uint32_t>(), words[kCodeShiftConstants], kPairs);
        DataCopy(idShifts_.Get<uint32_t>(), words[kIdShiftConstants], kPairs);
        DataCopy(pairOffsets_.Get<uint32_t>(), words[kPairConstants], kHalfTileBytes);
        Fence<HardEvent::MTE2_V>();
      } else {
        // Explicit legacy diagnostic entry points remain self-contained.
 ...
 private:
  ...
  __aicore__ inline void LoadM1(uint32_t start, uint32_t half) {
    if (half == 0) {
      // Four real 32-byte blocks land directly in NZ row zero. Remaining
      // rows were zeroed once for this output tile. AIV1 never reads x_.
      // Preserve the prior MTE3 -> V -> MTE2 reuse ordering of LoadNd.
      Fence<HardEvent::V_MTE2>();
      DataCopyParams p;
      p.blockCount = kK / kC0;
      p.blockLen = 1;
  ...
  __aicore__ inline void Vector(uint32_t group) {
    ...
    if (m_ == 1) {
      // Reinitialize for EVERY N tile/job: previous jobs may have M > 1.
      Duplicate(aUb_.Get<uint32_t>(), uint32_t(0), kHalfTileBytes / 4);
      Fence<HardEvent::V_MTE2>();
    }
...
#define VQ2_KERNEL(NAME, MODE)                                                                                     \
  extern "C" __global__ __aicore__ void NAME(GM_ADDR x, GM_ADDR scale, GM_ADDR bias, GM_ADDR packed, GM_ADDR book, \
    ...
    vq2a8_v3::ProjectionKernel<> op;                                                                               \
    op.Init(x, scale, bias, packed, book, ids, dense, y, m, n, k, tiles);                                          \
    op.Process<MODE>(cores);                                                                                       \
  }
VQ2_KERNEL(vq2a8_v3_direct, 0)
VQ2_KERNEL(vq2a8_v3_bridge, 1)
VQ2_KERNEL(vq2a8_v3_fused, 2)
#undef VQ2_KERNEL

extern "C" __global__ __aicore__ void vq2a8_v3_grouped(GM_ADDR descriptors, uint32_t jobs, uint32_t groups,
                                                       uint32_t cores) {
  KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
  vq2a8_v3::ProjectionKernel<> op;
  op.InitBuffers();
...
extern "C" __global__ __aicore__ void vq2a8_v3_grouped_pipeline(GM_ADDR descriptors, uint32_t jobs, uint32_t groups,
                                                                uint32_t cores) {
  KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
  vq2a8_v3::ProjectionKernel<true> op;
  op.InitBuffers();
  op.ProcessGrouped(descriptors, jobs, groups, cores);
}

extern "C" __global__ __aicore__ void vq2a8_v3_prepared(GM_ADDR descriptors, GM_ADDR constants, uint32_t jobs,
                                                        uint32_t groups, uint32_t cores) {
  KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
  vq2a8_v3::ProjectionKernel<> op;
  op.InitBuffers(constants);
  op.ProcessGrouped(descriptors, jobs, groups, cores);
}

extern "C" __global__ __aicore__ void vq2a8_v3_prepared_pipeline(GM_ADDR descriptors, GM_ADDR constants, uint32_t jobs,
                                                                 uint32_t groups, uint32_t cores) {
  KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
  vq2a8_v3::ProjectionKernel<true> op;
  op.InitBuffers(constants);
...
void LaunchGroupedV3(void* stream, uint32_t blocks, void* descriptors, void* constants, uint32_t jobs, uint32_t groups,
                     bool pipeline) {
  if (pipeline) {
    vq2a8_v3_prepared_pipeline<<<blocks, nullptr, stream>>>(static_cast<GM_ADDR>(descriptors),
                                                            static_cast<GM_ADDR>(constants), jobs, groups, blocks);
  } else {
    vq2a8_v3_prepared<<<blocks, nullptr, stream>>>(static_cast<GM_ADDR>(descriptors), static_cast<GM_ADDR>(constants),
                                                   jobs, groups, blocks);
  }
}
void LaunchGroupedPipeline(void* stream, uint32_t blocks, void* descriptors, uint32_t jobs, uint32_t groups) {
  vq2a8_v3_grouped_pipeline<<<blocks, nullptr, stream>>>(static_cast<GM_ADDR>(descriptors), jobs, groups, blocks);
}
void LaunchGrouped(void* stream, uint32_t blocks, void* descriptors, uint32_t jobs, uint32_t groups) {
  vq2a8_v3_grouped<<<blocks, nullptr, stream>>>(static_cast<GM_ADDR>(descriptors), jobs, groups, blocks);
}
...
#define VQ2_ARGS                                                                                                      \
  static_cast<GM_ADDR>(x), static_cast<GM_ADDR>(scale), static_cast<GM_ADDR>(bias), static_cast<GM_ADDR>(packed),     \
      ...
      n, k, tiles, blocks
  if (mode == 0) {
    vq2a8_v3_direct<<<blocks, nullptr, stream>>>(VQ2_ARGS);
  } else if (mode == 1) {
    vq2a8_v3_bridge<<<blocks, nullptr, stream>>>(VQ2_ARGS);
  } else {
    vq2a8_v3_fused<<<blocks, nullptr, stream>>>(VQ2_ARGS);
  }
#undef VQ2_ARGS
}
}  // namespace vq2a8_v3
