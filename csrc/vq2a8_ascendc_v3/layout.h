...
#pragma once
#include <cstdint>
#ifndef VQ2A8_V3_LAYOUT_FN
  #define VQ2A8_V3_LAYOUT_FN inline
#endif
...
// Prepared ABI 1 accepts int32[kConstantWords]. Offsets/counts are words,
// whereas the values inside the Gather tables are BYTE offsets as in v1.
constexpr uint32_t kAbiVersion = 1;
constexpr uint32_t kNdConstants = 0;
constexpr uint32_t kPackedConstants = kNdConstants + kHalfTileBytes / 4;
constexpr uint32_t kIdConstants = kPackedConstants + kPairs;
constexpr uint32_t kCodeShiftConstants = kIdConstants + kPairs;
constexpr uint32_t kIdShiftConstants = kCodeShiftConstants + kPairs;
constexpr uint32_t kPairConstants = kIdShiftConstants + kPairs;
constexpr uint32_t kConstantWords = kPairConstants + kHalfTileBytes;
static_assert(kConstantWords == 6656, "Prepared v3 constant ABI changed");
...
VQ2A8_V3_LAYOUT_FN constexpr uint32_t NzRow(uint32_t byte) { return (byte / kC0) % kHalf; }
VQ2A8_V3_LAYOUT_FN constexpr uint32_t NzCol(uint32_t byte) { return byte / (kHalf * kC0) * kC0 + byte % kC0; }
VQ2A8_V3_LAYOUT_FN constexpr uint32_t NdGatherOffset(uint32_t word) { return NzRow(word * 4) * kK + NzCol(word * 4); }
VQ2A8_V3_LAYOUT_FN constexpr uint32_t PackedGatherOffset(uint32_t pair) {
  return (pair / kK * (kK / 8) + pair % kK / 8) * 4;
}
VQ2A8_V3_LAYOUT_FN constexpr uint32_t IdGatherOffset(uint32_t pair) { return (pair % kK / 4) * 4; }
VQ2A8_V3_LAYOUT_FN constexpr uint32_t PairGatherOffset(uint32_t byte) {
  return (NzRow(byte) / 2 * kK + NzCol(byte)) * 2 + NzRow(byte) % 2;
}

// Host initialization runs once for a prepared workspace, never per AIV.
// Also used by ordinary C++ tests; do not invent a different Python formula.
VQ2A8_V3_LAYOUT_FN void FillConstantWords(uint32_t* words) {
  for (uint32_t i = 0; i < kHalfTileBytes / 4; ++i) words[kNdConstants + i] = NdGatherOffset(i);
  ...
  for (uint32_t i = 0; i < kHalfTileBytes; ++i) words[kPairConstants + i] = PairGatherOffset(i);
}
...
VQ2A8_V3_LAYOUT_FN constexpr uint32_t HalfRows(uint32_t rows, uint32_t firstRow) {
  if (rows <= firstRow) {
    return 0;
  }
...
VQ2A8_V3_LAYOUT_FN constexpr uint32_t HalfNz(uint32_t row, uint32_t col) {
  return (col / kC0) * kHalf * kC0 + row * kC0 + col % kC0;
}
VQ2A8_V3_LAYOUT_FN constexpr uint32_t FullNz(uint32_t row, uint32_t col) {
  return (col / kC0) * kN * kC0 + row * kC0 + col % kC0;
}
VQ2A8_V3_LAYOUT_FN constexpr uint32_t Code(uint32_t word, uint32_t col) { return (word >> ((col % 8) * 4)) & 15u; }
VQ2A8_V3_LAYOUT_FN constexpr uint32_t BookOffset(uint32_t tile, uint32_t code, uint32_t row) {
  return tile * 32 + code * 2 + row % 2;
}
VQ2A8_V3_LAYOUT_FN constexpr uint32_t PackedOffset(uint32_t row, uint32_t col, uint32_t k) {
...
VQ2A8_V3_LAYOUT_FN void DecodeFour(uint32_t codes, uint32_t tileIds, uint32_t tiles, const Reader& table,
...
VQ2A8_V3_LAYOUT_FN constexpr bool ValidDimensions(int64_t m, int64_t n, int64_t k, int64_t tiles) {
