...
static_assert(kUbBytes <= 256 * 1024 && kL1Bytes <= 512 * 1024, "VQ2A8 v3 resident tile exceeds UB/L1 capacity");
static_assert(kL0ABytes <= 64 * 1024 && kL0BBytes <= 64 * 1024 && kL0CBytes <= 256 * 1024,
              "VQ2A8 v3 resident tile exceeds L0 capacity");
static_assert(kAicK == 2 * kAivK && kAivK % kCodebookK == 0 && kAicK % kMadK == 0,
              "VQ2A8 v3 resident AIC/AIV/LUT reduction tiles disagree");
...
#ifndef VQ2_V3_RESIDENT_LAYOUT_FN
  #define VQ2_V3_RESIDENT_LAYOUT_FN constexpr
  #define VQ2_V3_RESIDENT_UNDEF_LAYOUT_FN
#endif
VQ2_V3_RESIDENT_LAYOUT_FN bool ValidDimensions(int64_t m, int64_t n, int64_t k) {
...
VQ2_V3_RESIDENT_LAYOUT_FN uint64_t PackedOffset(uint32_t nBegin, uint32_t kBegin, uint32_t k) {
  return (uint64_t(nBegin / kN0) * (k / kK0) + kBegin / kK0) * kK0 * (kN0 / 4);
}
VQ2_V3_RESIDENT_LAYOUT_FN uint64_t TableOffset(uint32_t nBegin, uint32_t kBegin, uint32_t n) {
  return (uint64_t(kBegin / kCodebookK) * (n / kN0) + nBegin / kN0) * 32;
}
VQ2_V3_RESIDENT_LAYOUT_FN uint32_t DecodedOffset(uint32_t n1, uint32_t k1) {
  return (n1 * (kAivK / kK0) + k1) * (kK0 + 1) * kN0;
}
VQ2_V3_RESIDENT_LAYOUT_FN uint32_t B1Offset(uint32_t n1, uint32_t half) { return (n1 * kAicK + half * kAivK) * kN0; }
#ifdef VQ2_V3_RESIDENT_UNDEF_LAYOUT_FN
  #undef VQ2_V3_RESIDENT_LAYOUT_FN
  #undef VQ2_V3_RESIDENT_UNDEF_LAYOUT_FN
#endif
}  // namespace vq2a8_v3_resident
