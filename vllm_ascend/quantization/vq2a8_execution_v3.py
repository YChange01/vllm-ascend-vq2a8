...
"""All-resident B1 dispatch with a V2 pair-LUT core in the V3 library.

The legacy V3 kernel stays available for diagnostics. The default resident path
converts compressed weights once, retains dense rowwise RHT/bias GEMV, and can
fuse quantization/permutation and capture complete MoE decode layers. K ordering
changes accumulation: V1 bitwise equivalence is measured, never presumed.
"""
...
from vllm_ascend.quantization.vq2a8_v3_graph import MoEDecodeGraph
from vllm_ascend.quantization.vq2a8_v3_workspace import (
    RESIDENT_FIELDS,
    ResidentV2ProjectionWorkspace,
    resident_shapes,
    resident_workspace_sizes,
)
...
def resident_plan(layers, budget_bytes, *, top_k=6, report_budget=False, kernel="v2"):
    ...
    """
    if kernel not in ("v2", "legacy"):
        raise ValueError("V3 resident kernel must be v2 or legacy.")
    ...
    for layer in layers:
        ...
        for kind in KINDS:
            ...
            if kernel == "v2":
                shapes = resident_shapes(layer, kind)
                payload_bytes += sum(_rounded(math.prod(shapes[field]) * size) for field, _, size in RESIDENT_FIELDS)
                workspace_bytes += sum(_rounded(size) for size in resident_workspace_sizes(experts, jobs, n, k))
                continue
            if not (
                0 < n <= 65536
                and n % 32 == 0
        ...
        # One H128 FP32 matrix and one cumulative validity flag per runtime.
        # The ID lookup is bounded by the router expert domain (<=256).
        workspace_bytes += _rounded(128 * 128 * 4) + _rounded(256 * 8) + _rounded(1)
        if kernel == "legacy":
            workspace_bytes += _rounded(CONSTANT_WORDS * 4)
    ...
    return dict(
        **totals,
        ...
        layout="zn_pair_lut_k256" if kernel == "v2" else "v1_packed",
...
class AscendCV3VQ2TP1MoE(AscendCVQ2TP1MoE):
    ...
    def __init__(self, *args, v3_preparation="eager", v3_decode_graph="none", projection_kernel="v2", **kwargs):
        ...
        if projection_kernel not in ("v2", "legacy"):
            raise ValueError("V3 projection kernel must be v2 or legacy.")
        if projection_kernel == "legacy" and (v3_preparation != "eager" or v3_decode_graph != "none"):
            raise ValueError("Legacy V3 diagnostic runtime requires eager preparation and no graph.")
        super().__init__(*args, **kwargs)
        self.projection_kernel = projection_kernel
        self.v3_preparation, self.v3_decode_graph = v3_preparation, v3_decode_graph
    ...
    def _retain_valid(self, valid):
        if self._resident_valid is None:
            self._resident_valid = valid.clone()
        else:
    ...
    def initialize_resident(self, *, budget_bytes):
        ...
        payload_bytes_per_expert = (
            sum(
                math.prod(resident_shapes(self.layer, kind)[field][1:]) * size
                for kind in KINDS
                for field, _, size in RESIDENT_FIELDS
            )
            if kernel == "v2"
            else sum(
        ...
        try:
            ...
            with torch.inference_mode(False):
                banks = {
                    kind: {
                        field: torch.empty(
                            (
                                resident_shapes(self.layer, kind)[field]
                                if kernel == "v2"
                                else self.layer.tensor_shapes[f"{kind}_{field}"]
                            ),
                            dtype=dtype,
                            device=self.device,
                        )
                        for field, dtype, _ in (
                            RESIDENT_FIELDS
                            if kernel == "v2"
                            else tuple(
                                (field, VQ2_TP1_TORCH_DTYPES[field], size) for field, size in ELEMENT_BYTES.items()
                            )
                        )
                    }
                    for kind in KINDS
            ...
            with torch.inference_mode(False):
                ...
                self._resident_valid = torch.ones((), dtype=torch.bool, device=self.device)
            ...
            if getattr(self, "v3_decode_graph", "none") == "moe":
                self._decode_graph = MoEDecodeGraph(self.device)
    ...
    def _immutable_tensors(self):
        for workspace in self._resident_workspaces.values():
            ...
            if hasattr(workspace, "constants"):
    ...
    def _bind_stream(self, current):
        ...
        if self._resident_valid is not None:
            owners.append(self._resident_valid)
        for workspace in self._resident_workspaces.values():
    ...
    def resident_report(self):
        ...
        kernel = getattr(self, "projection_kernel", "legacy")
        graph = getattr(self, "_decode_graph", None)
        return dict(
            ready=True,
            ...
            layout="zn_pair_lut_k256" if kernel == "v2" else "v1_packed",
            resident_abi_version=1 if kernel == "v2" else None,
            resident_projection_launches=self.decode_device_calls * 2 if kernel == "v2" else 0,
            resident_prefill_launches=getattr(self, "resident_prefill_launches", 0),
            decode_graph=graph.report()
            ...
            preparation=(
                "dense_rht_bias_gemv_fused_quant_permute"
                if getattr(self, "v3_preparation", "eager") == "fused"
                else "dense_rht_bias_gemv_eager_quant_permute"
                if kernel == "v2"
                else "v1_rowwise_dense_rht_no_fwht"
            ),
            preparation_mode=getattr(self, "v3_preparation", "eager"),
            ...
            route_host_reads=0,
            counter_scope="logical_decode_calls_excludes_capture_warmups_prefill_is_eager_host_routed",
        )
    ...
    def configure_v3_probe(self, *, measurement, profile=False, optimization=None, compact=True):
        ...
        if optimization not in (None, "v3", "batched"):
            raise ValueError("V3 probe supports only the fixed resident path; FWHT/pipeline are not implicit.")
        self.check_resident_immutable()
        self.measurement_mode = measurement
        self.trace_native = False
        if self._resident_valid is not None:
            self._resident_valid.fill_(True)
    ...
    def _projections_many(self, requests):
        # B>1 prefill keeps original row preparation and eager submission,
        # using the same converted banks and V2 core as resident decode.
        prepared = self._row_preparation.many(requests)
        if getattr(self, "projection_kernel", "legacy") == "v2":
            inputs = [
                (
                    gather_prepared_activation(values[0], payload["activation_order"]),
                    values[1].contiguous(),
                    values[2].contiguous(),
                    payload["packed_zn"],
                    payload["pair_lut"],
    ...
    def _forward(self, hidden, input_ids):
        ...
        try:
            ...
            graph = getattr(self, "_decode_graph", None)
            result = self._decode(hidden, input_ids) if graph is None else graph.run(self._decode, hidden, input_ids)
    ...
    def _decode(self, hidden, input_ids):
        """Device-only MoE computation; counters and guards stay outside capture."""
