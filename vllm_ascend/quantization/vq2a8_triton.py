...
def _vq2a8_tp1_m1_packed_gemm_portable_kernel(
    ...
    for start_k in range(0, SIZE_K, BLOCK_K):
        ...
        # The packed lookup operates on bytes so that codebook selection is
        # exact. Reinterpret the selected bytes as native E4M3 immediately
        # before Cube matmul. No MX block scale is involved; the model's
        # dynamic A8 scale is applied once in the epilogue.
    ...
    def _vq2a8_tp1_m1_packed_gemm_ascend_kernel(
        ...
        for start_k in range(0, SIZE_K, BLOCK_K):
            ...
            for source_tile in range(0, COLUMN_TILES):
                ...
                for code in range(0, CODEBOOK_SIZE):
                    # The full 32-byte table has already been transferred at
                    # an aligned GM address. get_element extracts from that
                    # on-chip tensor, avoiding dynamic one-byte MTE gathers.
    ...
    if activation.device.type == "npu":
        if _vq2a8_tp1_m1_packed_gemm_ascend_kernel is None:
            raise RuntimeError(
                ...
                "triton.language.extra.cann.extension, which is required for "
                "aligned codebook extraction in the VQ2A8 A5 kernel."
            )
