...
class FusedLayerValidity:
    ...
    def __init__(self, native_ops=None, *, reduction="vectorized"):
        if reduction != "vectorized":
            raise ValueError("Only vectorized layer validity is supported.")
        native = torch.ops.vq2a8_ascendc_v4_v2 if native_ops is None else native_ops
        operation_name = "layer_validity_vectorized"
        required_abi = LAYER_VALIDITY_VECTORIZED_ABI
        try:
