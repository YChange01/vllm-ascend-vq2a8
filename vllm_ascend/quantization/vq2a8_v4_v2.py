...
"""Resident ownership and exact accepted dispatch for the VQ2A8 packed layout.

Preparation remains in original K order; the native bank gathers FP8 bytes
after quantization. Weight layout conversion is startup-only, and eager
prefill and captured decode share the same vectorized projection backend.
"""
...
from vllm_ascend.quantization.vq2a8_config import validate_supported_modes
...
def require_v4_v2_features(*, native_ops=None):
    """Reject incomplete native builds before allocating expert weights."""
    ...
    for feature in (
        ...
        try:
            ...
            raise RuntimeError(f"VQ2A8 native library is missing {feature}; no fallback.") from error
        if type(version) is not int or version != 1:
            raise RuntimeError(f"Unsupported VQ2A8 {feature}={version!r}; require 1.")
...
def require_v4_v2_library():
    try:
        ...
        raise RuntimeError("Load libvq2a8_ascendc_v4_v2.so first; no fallback is enabled.") from error
    if type(abi) is not int or abi != V4_V2_ABI_VERSION:
        raise RuntimeError("V4 v2 resident library ABI mismatch.")
    return bank


def installed_v4_v2_library_path(package_root=None):
    """Resolve the native library installed by the package's normal build."""
    root = Path(__file__).resolve().parents[1] if package_root is None else Path(package_root)
    path = root / "libvq2a8_ascendc_v4_v2.so"
    if not path.is_file():
        raise RuntimeError(
            f"VQ2A8 native library is not installed at {path}. "
            "Rebuild vllm-ascend with VQ2A8 support; no fallback is enabled."
        )
    return path.resolve(strict=True)


def load_v4_v2_library(path=None, expected_sha256=None):
    """Load the installed binary, or an explicitly pinned validation binary."""
    if path is None:
        if expected_sha256 is not None:
            raise ValueError("A SHA256 override requires an explicit library path.")
        path = installed_v4_v2_library_path()
    else:
        if not isinstance(expected_sha256, str) or re.fullmatch(r"[0-9a-f]{64}", expected_sha256) is None:
            raise ValueError("An explicit VQ2A8 native library requires its SHA256 identity.")
        path = Path(path).resolve(strict=True)
    if not path.is_file() or path.name != "libvq2a8_ascendc_v4_v2.so":
        raise ValueError("VQ2A8 requires libvq2a8_ascendc_v4_v2.so.")
    with path.open("rb") as stream:
        actual = hashlib.file_digest(stream, "sha256").hexdigest()
    if expected_sha256 is not None and actual != expected_sha256:
        raise ValueError("VQ2A8 library SHA256 does not match the selected binary.")
    if str(path) not in torch.ops.loaded_libraries:
        if hasattr(torch.ops.vq2a8_ascendc_v4_v2, "abi_version"):
            raise RuntimeError("Another VQ2A8 library is registered; use a fresh process.")
...
class AscendCV4V2VQ2TP1MoE(AscendCV4VQ2TP1MoE):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        validate_supported_modes(
            {
                "v4_activation_reorder": v4_activation_reorder,
                "v4_activation_preparation": v4_activation_preparation,
                "v4_validity_mode": v4_validity_mode,
                "v4_route_mapping": v4_route_mapping,
                "v4_runtime_guard": v4_runtime_guard,
                "v4_select_sign": v4_select_sign,
                "v4_activation_tail": v4_activation_tail,
                "v4_b1_schedule": v4_b1_schedule,
                "v4_swiglu_mode": v4_swiglu_mode,
            }
        )
    ...
    def make_v4_preparation(self, *, compact=False, validity=None):
        ...
        return PackedRowwiseVQ2A8Preparation(compact=compact, validity=validity)

    def project_v4_prepared(self, bank, quantized, scale, bias, slots):
        """Use the same vectorized reorder for eager prefill and captured decode."""
    ...
    def _create_device_route_banks(self):
        ...
        for kind in ("gate_up", "down"):
            ...
            if not hasattr(bank, "project_vectorized") or not hasattr(bank, "select_sign"):
                raise RuntimeError("VQ2A8 resident bank lacks the accepted projection/select-sign ABI.")
