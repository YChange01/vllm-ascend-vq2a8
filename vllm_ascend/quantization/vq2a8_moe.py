...
"""Shared TP1 routing, root weights and eager reference behavior.

The V4/v2 runtime overrides projection with compressed AscendC execution.
The base projection is a CPU-only dense oracle, never a device fallback.
"""
...
class VQ2TP1MoE:
    ...
    def __init__(
        ...
        *,
        ...
        if self.device.type not in ("cpu", "npu"):
            raise ValueError("The TP1 MoE supports the CPU reference or Ascend NPU only.")
    ...
    def _projection(self, hidden: torch.Tensor, payload: dict, spec) -> torch.Tensor:
        ...
        raise RuntimeError(
            "NPU projection requires the V4/v2 AscendC execution policy; no device fallback is provided."
        )
