...
"""Bounded prefill routing and device-only singleton decode configuration."""
...
def configure_runtime(runtime, preset="device_route_decode", *, profile=False):
    """Configure the accepted resident execution path at a fenced boundary."""
    from vllm_ascend.quantization.vq2a8_v4_device_route import DeviceRouteDecodeState

    if preset != "device_route_decode":
        raise ValueError("Only device_route_decode is supported; no experimental preset fallback.")
    if not hasattr(runtime, "_optimization_states"):
    ...
    if preset not in runtime._optimization_states:
        ...
        preparation = runtime.make_v4_preparation(compact=True, validity=state.retain)
