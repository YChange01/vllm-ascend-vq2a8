...
本分支基于 vLLM Ascend v0.23.0，先按 [v0.23 迁移说明](vq2a8_v023_migration.md)
建立独立环境。已有 TP2 ZN 文件可复用，不因框架迁移重新 repack。
...
默认每次通信测试前都要求 `npu-smi` 明确显示两卡无进程；占用、查询失败或无法识别时跳过，
环境检查仍继续。空闲检查只是快照，不是独占预留。

如果确认允许与已有任务共享卡 0、1，可显式开启：

```bash
bash tools/vq2_tp2_diagnose.sh --allow-busy
```

此选项仅放行可明确识别的占用状态；查询失败、缺卡、无法识别或矛盾状态仍跳过。
每次测试仍收集进程快照；共享卡测试打印 `TEST_ISOLATION=SHARED_DEVICE` 和警告。
两种模式都只做小张量通信检查，不加载模型或 VQ2A8 自定义算子库，不停止已有任务。
但 NPU 上下文和 HCCL 本身需要额外 HBM/通信资源，并非只占张量的几十字节；
共享测试可能影响已有任务延迟，也可能因资源竞争失败。`TIMING_VALID=False` 表示
这些检查不能当作 TPOT/带宽基准；共享状态下失败还需在空闲环境复核。
可用 `--help` 查看参数，不会启动任何检查。
